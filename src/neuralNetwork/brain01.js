  ...
  learning(fitness, duration) {
    ...
    if (!bestNetwork) {
      ...
      if (random < 0.23) {
        ...
        this.randomisation(this.neuronsPros, 0.05, 0.001 * varianceMax)
      } else if (random < 0.3) {
        this.neuronsPros = bestNetwork.slice()
        this.randomisation(this.neuronsPros, 0.1, 0.05 * varianceMax)
      } else if (random < 0.75) {
        this.neuronsPros = bestNetwork.slice()
        this.randomisation(this.neuronsPros, 0.2, 0.1 * varianceMax)
      } else if (random < 0.95) {
        this.neuronsPros = bestNetwork.slice()
        this.randomisation(this.neuronsPros, 0.2, 0.5 * varianceMax)
      } else if (random < 0.97) {
        this.neuronsPros = bestNetwork.slice()
        this.randomisation(this.neuronsPros, 1, varianceMax * 2)
      } else {
        this.fullRandomisation(this.neuronsPros, varianceMax * 4)
      }
...
const guiFolder = gui.addFolder('For this network configuration')
guiFolder.add(Brain, 'loadBestNetwork').name('load current state in memory');
guiFolder.add(Brain, 'saveBestNetwork').name('save current state  in memory');
